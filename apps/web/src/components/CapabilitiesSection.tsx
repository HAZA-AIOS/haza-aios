        ...
        <div className="grid gap-6 md:grid-cols-2 xl:grid-cols-3">
          {capabilityItems.map((item, index) => {
            ...
            return (
              <Card
                key={item.title}
                className="group relative overflow-hidden border-none bg-[#0a0a0f] min-h-[450px] p-0 transition-all duration-500 hover:-translate-y-2 hover:shadow-[0_20px_40px_rgba(0,0,0,0.8)] flex flex-col items-center justify-center rounded-sm"
                data-aos="fade-up"
                data-aos-delay={index * 60}
              >
                {/* Background glowing waves simulation */}
                <div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[120%] h-[120%] opacity-[0.25] blur-[80px] rounded-full bg-gradient-to-br ${colors[index % colors.length]} transition-opacity duration-500 group-hover:opacity-40`} />
                ...
                <div className="absolute top-6 left-6 flex flex-col gap-1 z-20">
                  <div className="w-1.5 h-1.5 bg-white/70 rounded-full shadow-[0_0_5px_rgba(255,255,255,0.5)]" />
                  <div className="w-1.5 h-1.5 bg-white/70 rounded-full shadow-[0_0_5px_rgba(255,255,255,0.5)]" />
                  <div className="w-1.5 h-1.5 bg-white/70 rounded-full shadow-[0_0_5px_rgba(255,255,255,0.5)]" />
                  <span className="text-[7px] text-white/50 tracking-[0.3em] uppercase mt-3">HAZA AIOS PRESENTS</span>
                </div>

                {/* Top Right Pill (Badge) */}
                <div className="absolute top-6 right-6 flex flex-col items-end z-20">
                  <span className="text-[7px] text-white/50 tracking-[0.2em] uppercase mb-2">INDEX</span>
                  <div className={`flex items-center justify-center w-10 h-10 rounded-full bg-gradient-to-br ${colors[index % colors.length]} shadow-[0_5px_20px_rgba(0,0,0,0.5)] border border-white/20 backdrop-blur-md`}>
                    <span className="text-white font-black text-sm drop-shadow-md">{`0${index + 1}`}</span>
                  </div>
                </div>

                {/* Right Edge Rotated Text */}
                <div className="absolute -right-4 top-1/2 -translate-y-1/2 -rotate-90 origin-center opacity-30 z-20">
                  <span className="text-[8px] tracking-[0.4em] text-white uppercase whitespace-nowrap">CAPABILITY FEATURE</span>
                </div>

                {/* Center Geometric Shape */}
                <div className={`relative flex items-center justify-center w-[220px] h-[220px] border border-white/20 z-10 transition-transform duration-700 group-hover:border-white/40 group-hover:scale-105 shadow-[0_0_30px_rgba(0,0,0,0.3)] bg-black/10 backdrop-blur-[2px] ${currentShape}`}>
                  <div className={`absolute inset-0 flex flex-col items-center justify-center p-6 text-center ${isDiamond ? '-rotate-45' : ''}`}>
                    <h3 className="text-3xl font-black text-white leading-[1.1] tracking-tight uppercase mb-2 drop-shadow-[0_2px_10px_rgba(0,0,0,0.8)]">
