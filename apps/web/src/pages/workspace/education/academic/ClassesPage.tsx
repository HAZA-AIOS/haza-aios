...
import { Plus, Layers, X, AlertCircle, ArrowLeft, Edit2, Trash2 } from "lucide-react";
import { AcademicService } from "@/modules/education/sis/academic.service";
import type { Grade } from "@/modules/education/sis/sis.types";
...
export const ClassesPage: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    setLoading(true);
    const data = await AcademicService.getGrades(currentOrganization.id);
    setGrades(data);
    setLoading(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await AcademicService.createGrade(currentOrganization.id, formData);
      setShowModal(false);
      setFormData({ name: "", level: 1, order: grades.length + 1, status: "active" });
      loadData();
    } catch (err: any) {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white flex items-center gap-2">
                <Layers className="w-6 h-6 text-purple-500" /> Grades & Classes
              </h1>
              <p className="text-slate-400">Define the grade levels offered at your institution.</p>
            </div>
          </div>
          <Button onClick={() => {
              ...
              <table className="w-full text-sm text-left text-slate-300">
                <thead className="text-xs uppercase bg-slate-900/50 text-slate-400">
                  <tr>
                    ...
                    <th className="px-6 py-4 font-medium">Academic Level</th>
                ...
                <tbody className="divide-y divide-white/5">
                  {grades.map(grade => (
                    <tr key={grade.id} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-6 py-4 text-slate-400">{grade.order}</td>
                      <td className="px-6 py-4 font-medium text-white">{grade.name}</td>
                      <td className="px-6 py-4">Level {grade.level}</td>
                      <td className="px-6 py-4">
                        <Badge variant={getStatusColor(grade.status) as any}>{grade.status}</Badge>
                      </td>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 animate-in fade-in">
          <div className="bg-[#0f141f] border border-white/10 rounded-xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95">
            <div className="flex justify-between items-center p-4 border-b border-white/5">
              <h2 className="text-lg font-bold text-white">Add Grade / Class</h2>
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              ...
              <div className="space-y-1">
                <label className="text-sm font-medium text-slate-300">Grade Name</label>
                <Input 
                  placeholder="e.g. Grade 1, Year 10" 
                  value={formData.name}
