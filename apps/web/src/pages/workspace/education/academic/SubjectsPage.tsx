...
import { Plus, BookOpen, X, AlertCircle, ArrowLeft, Edit2, Trash2 } from "lucide-react";
...
export const SubjectsPage: React.FC = () => {
  ...
  const loadData = async () => {
    ...
    setLoading(true);
    const data = await AcademicService.getSubjects(currentOrganization.id);
    setSubjects(data);
    setLoading(false);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await AcademicService.createSubject(currentOrganization.id, formData);
      setShowModal(false);
      setFormData({ name: "", code: "", description: "", displayOrder: subjects.length + 1, status: "active" });
      loadData();
    } catch (err: any) {
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-white flex items-center gap-2">
                <BookOpen className="w-6 h-6 text-orange-500" /> Subjects
              </h1>
              <p className="text-slate-400">Master catalog of courses and subjects.</p>
            </div>
          </div>
          <Button onClick={() => {
            setFormData(prev => ({ ...prev, displayOrder: subjects.length + 1 }));
            setShowModal(true);
          }}>
          ...
          <CardContent className="p-0">
            {loading ? (
              ...
              <div className="py-12 text-center text-slate-500">
                No subjects defined. Click "Add Subject" to start.
              </div>
            ) : (
                ...
                <tbody className="divide-y divide-white/5">
                  {subjects.map(subject => (
                    <tr key={subject.id} className="hover:bg-white/[0.02] transition-colors">
                      <td className="px-6 py-4 font-mono text-slate-400">{subject.code}</td>
                      <td className="px-6 py-4 font-medium text-white">{subject.name}</td>
                      <td className="px-6 py-4 text-slate-500 max-w-xs truncate">{subject.description || "-"}</td>
                      <td className="px-6 py-4 text-slate-400">{subject.displayOrder}</td>
                      <td className="px-6 py-4">
                        <Badge variant={getStatusColor(subject.status) as any}>{subject.status}</Badge>
                      </td>
            ...
            <form onSubmit={handleSubmit} className="p-4 space-y-4">
              ...
              <div className="space-y-1">
                ...
                <Input 
                  ...
                  required
                  className="bg-slate-900 border-white/10 text-white font-mono uppercase"
                />
              </div>
              ...
              <div className="space-y-1">
                ...
                <textarea 
                  ...
                  className="w-full bg-slate-900 border border-white/10 rounded-md p-2 text-white outline-none focus:border-blue-500/50 min-h-[80px]"
                />
              </div>
