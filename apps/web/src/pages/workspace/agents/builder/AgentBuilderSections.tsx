...
export const KnowledgeConfig: React.FC<SectionProps> = ({ config, onChange }) => {
  ...
  React.useEffect(() => {
    // In a real app we'd fetch for the current org
    KnowledgeService.getKnowledgeSources("org-1").then(res => {
      setSources(res);
      setLoading(false);
    });
  }, []);
  ...
  return (
    <BuilderSection title="Knowledge" description="Select the organizational knowledge sources this agent is authorized to retrieve.">
      {loading ? (
        ...
        <div className="space-y-3">
          {sources.map(source => {
            ...
            return (
              <div key={source.id} className="flex items-center justify-between p-4 border rounded-md">
                <div>
                  <h4 className="font-medium flex items-center gap-2">
                    {source.name}
                    <span className="text-[10px] uppercase tracking-wider bg-purple-500/10 text-purple-400 px-2 py-0.5 rounded">
                      {source.type}
                    </span>
                  </h4>
          ...
          {sources.length === 0 && (
            <div className="text-sm text-slate-400 italic">No authorized knowledge sources found in your organization.</div>
          )}
        </div>
      )}
    </BuilderSection>
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-between border-b pb-4">
          <div>
            ...
            <div className="text-sm text-muted-foreground">Include recent messages in prompt context</div>
          </div>
          <Switch aria-checked={config.memory.conversationContext} onClick={() => onChange({ memory: { ...config.memory, conversationContext: !config.memory.conversationContext } })} />
        </div>
        <div className="flex items-center justify-between pb-2">
          <div>
            <div className="font-medium">Persistent Memory Extraction</div>
            <div className="text-sm text-muted-foreground">Automatically extract and save user preferences from chat</div>
          </div>
          <Switch aria-checked={config.memory.persistentMemory} onClick={() => onChange({ memory: { ...config.memory, persistentMemory: !config.memory.persistentMemory } })} />
        </div>
      </div>
    </BuilderSection>
  </div>
